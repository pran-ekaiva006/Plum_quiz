import { QuizPayloadSchema, QuizPayload, FeedbackSchema, Feedback } from '../types/quiz'
import { safeJsonParse, sleep } from '../utils'

// --- Prompt Templates ---
export const QUIZ_JSON_PROMPT = (topic: string) => `
...
export const FEEDBACK_PROMPT = (topic: string, score: number) => `
...
// --- Public API ---
export async function generateQuiz(topic: string): Promise<QuizPayload> {
  if (USE_MOCK) {
    const mock: QuizPayload = {
      topic,
  ...
  const endpoint = (import.meta as any).env?.VITE_AI_ENDPOINT ?? 'http://localhost:3001/api/generate'
  const apiKey = (import.meta as any).env?.VITE_AI_API_KEY
  if (!endpoint || !apiKey) throw new Error("AI endpoint or API key missing. Use .env or set VITE_USE_MOCK=true")
...
export async function generateFeedback(topic: string, score: number): Promise<Feedback> {
  if (USE_MOCK) {
    await sleep(400)
    return { score, message: score >= 3 ? "Nice work! You've got a solid grasp—review the misses and try again. Tip: note tricky terms in a mini cheatsheet." : "Good start! Revisit the basics and retry. Tip: read each option aloud and eliminate two wrong choices first." }
  }
